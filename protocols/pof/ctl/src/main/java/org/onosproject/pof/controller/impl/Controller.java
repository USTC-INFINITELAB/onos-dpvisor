...
*/
/*
import org.projectfloodlight.openflow.protocol.OFFactory;//wenjian
*/

//import static org.onosproject.openflow.controller.Dpid.uri;//wenjian
...
public class Controller {
    ...
    wenjian*/
    protected static final BasicFactory FACTORY = new BasicFactory();
    //wenjian
    ...
    protected List<Integer> openFlowPorts = ImmutableList.of(6643);
    ...
    */
    ...
    private ServerBootstrap createServerBootStrap() {
        ...
        if (workerThreads == 0) {
            execFactory = new NioServerSocketChannelFactory(
                    Executors.newCachedThreadPool(groupedThreads("onos/pof", "boss-%d", log)),
                    Executors.newCachedThreadPool(groupedThreads("onos/pof", "worker-%d", log)));
            return new ServerBootstrap(execFactory);
        } else {
            execFactory = new NioServerSocketChannelFactory(
                    Executors.newCachedThreadPool(groupedThreads("onos/pof", "boss-%d", log)),
                    Executors.newCachedThreadPool(groupedThreads("onos/pof", "worker-%d", log)), workerThreads);
            return new ServerBootstrap(execFactory);
        }
    }

    public void setConfigParams(Dictionary<?, ?> properties) {
        String ports = get(properties, "pofPorts");
        if (!Strings.isNullOrEmpty(ports)) {
            this.openFlowPorts = Stream.of(ports.split(","))
                                       .map(s -> Integer.parseInt(s))
                                       .collect(Collectors.toList());
        }
        log.debug("POF ports set to {}", this.openFlowPorts);
    ...
    protected OpenFlowSwitchDriver getOFSwitchInstance(long dpid,
                                                       ...
                                                       OFVersion ofv) {
                                                       wenjian*/
    protected PofSwitchDriver getOFSwitchInstance(long dpid,
                                                  OFDescriptionStatistics desc) {
        Dpid dpidObj = new Dpid(dpid);

        Driver driver;
        try {
            driver = driverService.getDriver(DeviceId.deviceId(uri(dpidObj)));
        } catch (ItemNotFoundException e) {
            driver = driverService.getDriver(desc.getManufacturerDescription(), desc
                    .getHardwareDescription(), desc.getSoftwareDescription());
        }

        if (driver != null && driver.hasBehaviour(PofSwitchDriver.class)) {
            Dpid did = new Dpid(dpid);
            DefaultDriverHandler handler =
                    new DefaultDriverHandler(new DefaultDriverData(driver, DeviceId.deviceId(uri(did))));
            PofSwitchDriver pofSwitchDriver =
                    driver.createBehaviour(handler, PofSwitchDriver.class);
            pofSwitchDriver.init(did, desc);
            pofSwitchDriver.setAgent(agent);
            pofSwitchDriver.setRoleHandler(new RoleManager(pofSwitchDriver));
            log.info("POF handshaker found for device {}: {}", dpid, pofSwitchDriver);
            return pofSwitchDriver;
        }
        log.error("No POF driver for {} : {}", dpid, desc);
        return null;

    }

    public void start(PofAgent ag, DriverService driverSerVice) {
        log.info("Starting POF IO");
        this.agent = ag;
